<template>
  <div class="page-container">
    <div class="page-header">
      <div><h2>👋 下午好，{{ auth.user?.realName || '老师' }}</h2><p>今天是 {{ today }}，欢迎使用智学AI教育</p></div>
    ...
    <el-row :gutter="20">
      <el-col :span="12"><el-card><template #header><span class="card-title">📚 学科中心</span></template><div class="subject-grid"><div v-for="s in allSubjectList" :key="s.value" class="subject-chip" :class="{disabled:!s.active}" @click="s.active && $router.push(`/teacher/subject/${s.value}`)">{{ s.icon }} {{ s.label }}<span v-if="!s.active" class="chip-dev">待开发</span></div></div></el-card></el-col>
      <el-col :span="12"><el-card><template #header><span class="card-title">📊 今日概览</span></template><div class="today-item" v-for="t in todayStats" :key="t.label"><span>{{ t.label }}</span><strong>{{ t.value }}</strong></div></el-card></el-col>
    </el-row>
  </div>
</template>
<script setup>
...
const allSubjectList = ['数学','语文','英语','物理','化学','生物','历史','政治','地理'].map(s => ({ value:zh2en[s], label:s, icon:icons[s]||'📚', active: s==='数学' }))
